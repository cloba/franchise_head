...
INSERT INTO `contract` (`contract_code`, `head_staff_id`, `store_code`, `contractor_name`, `contract_actual_date`, `contract_regit_date`, `contract_expiry_date`, `contract_deposit`, `contract_profit_percent`, `contract_file`, `contract_n`, `contract_expire`, `contract_expired_date`, `contract_expire_content`) VALUES
	('contract_code1', 'head_staff_id1', 'store_code1', '김철희', '2016-08-01 14:49:59', '2016-08-02 14:50:00', '2016-08-02 14:50:01', 0, 0, '0', 1, 'N', '2016-08-02 14:50:12', NULL),
	('contract_code2', 'head_staff_id1', 'store_code2', '김철수', '2016-08-02 14:49:59', '2016-08-02 14:50:00', '2016-08-02 14:50:01', 0, 0, '0', 1, 'N', '2016-08-02 14:50:12', NULL);
...
-- Dumping data for table franchise_head.delivery: ~0 rows (approximately)
...
INSERT INTO `head_client` (`head_client_code`, `head_client_name`, `head_client_regit_date`, `head_client_in_charge`, `head_client_contract`, `head_client_phone_first`, `head_client_phone_second`, `head_client_phone_third`, `head_client_road_addr`, `head_client_post`, `head_client_parcle_addr`, `head_staff_id`) VALUES
	('head_client_code1', '홍길동', '2016-08-02 15:36:05', '허균', '계약중', '010', '2438', '2366', '익산시', '956741', '13번지', 'id001'),
	('head_client_code2', '홍길군', '2016-08-02 15:37:35', '허준', '계약중', '010', '3138', '3377', '전주시', '123330', '55번지', 'id001'),
	('head_client_code3', '하하', '2016-08-02 15:38:05', '허참', '계약완료', '010', '1234', '5678', '전주시', '123113', '77번지', 'id002');
...
CREATE TABLE IF NOT EXISTS `head_out` (
  ...
  `h_item_code` varchar(50) NOT NULL COMMENT '상품코드',
  `store_code` varchar(50) NOT NULL COMMENT '가맹일련번호',
  `head_out_date` datetime NOT NULL COMMENT '보내준 날짜',
...
-- Dumping data for table franchise_head.head_out: ~0 rows (approximately)
...
-- Dumping data for table franchise_head.head_sell: ~0 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `profit_by_pay_method` (
  ...
  `profit_percent` int(11) NOT NULL COMMENT '수익 %',
  `head_staff_id` varchar(50) NOT NULL COMMENT '등록한 직원 아이디',
...
CREATE TABLE IF NOT EXISTS `royalty` (
  `royalty_code` varchar(50) NOT NULL COMMENT '로열티지불 코드',
  `contract_code` varchar(50) NOT NULL COMMENT '계약코드',
  `royalty_dealine` date NOT NULL COMMENT '로열티 지급 기일',
  `royalty_paid` char(1) NOT NULL COMMENT '로열티 지급 여부',
  `royalty_expected_amount` int(11) NOT NULL COMMENT '해당월의 지급예정액',
  `royalty_pay_actual_date` date DEFAULT NULL COMMENT '실제 지급 날짜',
  `royalty_actual_amount` int(11) DEFAULT NULL COMMENT '해당월에 실제 지급한 금액',
  PRIMARY KEY (`royalty_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='로열티 실제 지불관리';

-- Dumping data for table franchise_head.royalty: ~0 rows (approximately)
/*!40000 ALTER TABLE `royalty` DISABLE KEYS */;
/*!40000 ALTER TABLE `royalty` ENABLE KEYS */;


-- Dumping structure for table franchise_head.royalty_expected
CREATE TABLE IF NOT EXISTS `royalty_expected` (
  `royalty_expected_code` varchar(50) NOT NULL COMMENT '로열티 지급예정 코드',
  `contract_code` varchar(50) NOT NULL COMMENT '계약코드',
  `royalty_expected_status` int(11) NOT NULL COMMENT '상태(일부납부/완불/지급예정)',
  `royalty_expected_paid` int(11) NOT NULL COMMENT '지급 금액',
  `royalty_expected_pay_date` date NOT NULL COMMENT '납부 일자',
  `royalty_expected_rest` int(11) NOT NULL COMMENT '남은 금액',
  PRIMARY KEY (`royalty_expected_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='로열티 지급 예정관리';

-- Dumping data for table franchise_head.royalty_expected: ~0 rows (approximately)
/*!40000 ALTER TABLE `royalty_expected` DISABLE KEYS */;
/*!40000 ALTER TABLE `royalty_expected` ENABLE KEYS */;
...
INSERT INTO `sub` (`sub_code`, `head_staff_id`, `sub_regit_date`, `sub_name`, `sub_owner`, `sub_phone_first`, `sub_phone_second`, `sub_phone_third`, `sub_post`, `sub_road_addr`, `sub_parcle_addr`) VALUES
	('sub_code1', 'head_staff_id1', '2016-08-02 11:06:15', '전주사랑점', '김철수', 10, 0, 0, 123456, '전주시덕진구', '전주시덕준구'),
	('sub_code2', 'head_staff_id1', '2016-08-02 11:06:15', '전주사랑점', '김철수', 10, 0, 0, 123456, '전주시덕진구', '전주시덕준구');
